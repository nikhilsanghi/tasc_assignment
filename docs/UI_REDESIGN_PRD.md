...
**Visual iteration is mandatory, not optional.** At every step: open the app in the in-app browser (`preview_start` on the dev server, port 3000), take a screenshot of what you built, put it side by side with the relevant reference screenshot in `private/Pin_Screenshots/`, and fix the differences — spacing, border weight, chip shape, badge color, section-label typography — before committing. Implementing the spec without looking at the rendered result is how a UI ends up technically correct and visually wrong. Budget at least one look-compare-adjust loop per view; more for the candidate card and drawer (the two highest-visibility surfaces).
